...
namespace GAME.FlowSys
{
    ...
    public class MissionTextUIManager : MonoBehaviour
    {
        ...
        public void ShowUI(string text, bool showCanvas, bool showTextPanel, bool showButtonsPanel, IList<MissionUIButtonConfiguration> buttonConfigurations, bool enableTypewriterEffect = true)
        {
            ...
            {
                Debug.LogError("[FlowSys] MissionTextUIManager is not configured: Canvas not found.");
                return;
            }
        ...
        public bool BindButton(MissionUIButtonType buttonType, Action callback, string userActionId = null)
        {
            ...
            {
                Debug.LogError($"[FlowSys] Cannot bind button '{buttonType}': button not found on MissionTextUIManager.");
                return false;
            }
        ...
        public void EnsureCanvasActive()
        {
            ...
            if (canvasToActivate != null)
            {
                ...
                Debug.Log($"[FlowSys] EnsureCanvasActive: canvas '{canvasToActivate.gameObject.name}' activated");
            }
            else
            {
                Debug.LogWarning("[FlowSys] EnsureCanvasActive: no Canvas found on UIManager — assign targetCanvas in the Inspector");
            }
        }
        ...
        public MultipleChoiceDialog GetMultipleChoiceDialog()
        {
            ...
            if (multipleChoiceDialog != null)
            {
                Debug.Log($"[FlowSys] GetMultipleChoiceDialog: using dialog '{multipleChoiceDialog.gameObject.name}' on manager '{gameObject.name}'");
            }
            else
            {
                Debug.LogWarning($"[FlowSys] GetMultipleChoiceDialog: not found. targetCanvas={(targetCanvas != null ? targetCanvas.gameObject.name : "NULL")}, manager={gameObject.name}");
            }
