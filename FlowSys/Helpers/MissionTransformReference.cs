...
namespace GAME.FlowSys
{
    ...
    public class MissionTransformReference : MonoBehaviour
    {
        ...
        private void TryRegister()
        {
            ...
            {
                Debug.LogWarning($"[FlowSys] MissionTransformReference on '{gameObject.name}' has no referenceId");
                return;
            }
    ...
    public class MissionTransformReferenceRegistry : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (instance != this)
                Debug.LogWarning("[FlowSys] Multiple MissionTransformReferenceRegistry instances detected");
        }
        ...
        public void RegisterReference(string id, MissionTransformReference reference)
        {
            ...
            {
                Debug.LogWarning("[FlowSys] Cannot register transform reference: id is empty");
                return;
            }

            if (reference == null)
            {
                Debug.LogWarning($"[FlowSys] Cannot register transform reference '{id}': reference is null");
                return;
            }

            if (references.ContainsKey(id) && references[id] != reference)
                Debug.LogWarning($"[FlowSys] Transform reference '{id}' already registered, overwriting");
        ...
        public MissionTransformReference GetReference(string id)
        {
            if (string.IsNullOrWhiteSpace(id))
            {
                Debug.LogWarning("[FlowSys] Cannot get transform reference: id is empty");
                return null;
            }
