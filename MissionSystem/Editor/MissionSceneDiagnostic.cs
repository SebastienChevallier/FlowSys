...
namespace GAME.MissionSystem.Editor
{
    ...
    {
        [MenuItem("GAME/Mission System/List All Root GameObjects")]
        ...
        [MenuItem("GAME/Mission System/Diagnose Scene Setup")]
        public static void DiagnoseScene()
        {
            // GameFlowManager
            GameFlowManager gfm = Object.FindObjectOfType<GameFlowManager>(true);
            if (gfm != null)
                Debug.Log($"[Diagnostic] GameFlowManager FOUND: '{gfm.gameObject.name}' active={gfm.gameObject.activeInHierarchy}");
            else
                Debug.LogWarning("[Diagnostic] GameFlowManager NOT FOUND in scene");
