...
{
    [MissionConditionEditorRenderer(typeof(ForceAdvanceOnTriggerPressedMissionConditionData))]
    internal sealed class ForceAdvanceOnTriggerPressedMissionConditionEditorRenderer : IMissionConditionEditorRenderer, IMissionUIConditionElementRenderer
    {
        public bool Draw(MissionStepConditionEntry condition, MissionInlineEditorContext context)
        {
            if (condition?.managedCondition is not ForceAdvanceOnTriggerPressedMissionConditionData data)
                return false;
        ...
        {
            if (entry?.managedCondition is not ForceAdvanceOnTriggerPressedMissionConditionData data)
                return null;
