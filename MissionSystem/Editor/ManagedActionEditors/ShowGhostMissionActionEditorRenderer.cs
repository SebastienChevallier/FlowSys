...
{
    [MissionActionEditorRenderer(typeof(ShowGhostMissionActionData))]
    internal sealed class ShowGhostMissionActionEditorRenderer : IMissionActionEditorRenderer, IMissionUIElementRenderer
    {
        public bool Draw(MissionStepActionEntry action, MissionInlineEditorContext context)
        {
            if (action?.managedAction is not ShowGhostMissionActionData data)
                return false;
