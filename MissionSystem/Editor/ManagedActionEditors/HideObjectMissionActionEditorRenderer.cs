...
{
    [MissionActionEditorRenderer(typeof(HideObjectMissionActionData))]
    internal sealed class HideObjectMissionActionEditorRenderer : IMissionActionEditorRenderer, IMissionUIElementRenderer
    {
        public bool Draw(MissionStepActionEntry action, MissionInlineEditorContext context)
        {
            if (action?.managedAction is not HideObjectMissionActionData data)
                return false;
