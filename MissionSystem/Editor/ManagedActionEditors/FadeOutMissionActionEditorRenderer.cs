...
{
    [MissionActionEditorRenderer(typeof(FadeOutMissionActionData))]
    internal sealed class FadeOutMissionActionEditorRenderer : IMissionActionEditorRenderer, IMissionUIElementRenderer
    {
        public bool Draw(MissionStepActionEntry action, MissionInlineEditorContext context)
        {
            if (action?.managedAction is not FadeOutMissionActionData data)
                return false;
