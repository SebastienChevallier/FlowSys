...
{
    [MissionActionEditorRenderer(typeof(DisableUserActionMissionActionData))]
    internal sealed class DisableUserActionMissionActionEditorRenderer : IMissionActionEditorRenderer, IMissionUIElementRenderer
    {
        public bool Draw(MissionStepActionEntry action, MissionInlineEditorContext context)
        {
            if (action?.managedAction is not DisableUserActionMissionActionData data)
                return false;
