...
{
    [MissionActionEditorRenderer(typeof(SetAnimatorBoolMissionActionData))]
    internal sealed class SetAnimatorBoolMissionActionEditorRenderer : IMissionActionEditorRenderer, IMissionUIElementRenderer
    {
        public bool Draw(MissionStepActionEntry action, MissionInlineEditorContext context)
        {
            if (action?.managedAction is not SetAnimatorBoolMissionActionData data)
                return false;
            ...
            data.animatorBoolValue = EditorGUILayout.Toggle("Bool Value", data.animatorBoolValue);
        ...
        public VisualElement BuildElement(MissionStepActionEntry action, MissionInlineEditorContext context)
        {
            ...
            root.Add(MissionGraphEditorUIComponents.CreateToggleField(
                "Bool Value",
                data.animatorBoolValue,
                value =>
                {
                    data.animatorBoolValue = value;
                    context?.SaveGraphPositions?.Invoke();
                    context?.Repaint?.Invoke();
                },
                paramTextColor,
